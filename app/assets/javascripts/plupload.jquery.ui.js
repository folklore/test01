	...
	*/
	...
	_enableRenaming: function() {
		...
		this.filelist.dblclick(function(e) {
			...
			nameInput = $('<input class="plupload_file_rename" required type="text" />').width(nameSpan.width()).insertAfter(nameSpan.hide());
			nameInput.val(name).blur(function() {
				...
				if ($.inArray(e.keyCode, [13, 27]) !== -1) {
					...
					if (e.keyCode === 13) {
						file.name = ( nameInput.val().trim().length > 0 ? nameInput.val() : name ) + ext;
